...
def main(cats, model_name, prompt_id, eval_method, score_type, temperature, p):
    ...
    for pi in prompt_id:
        ...
        for k in results.keys():
            results[k].to_csv(f"results/{model_path}_temp_{temperature.replace('.', '_')}/{k}.csv", index=False)
            
        scores = scoring_func(score_type, f"results/{model_path}_temp_{temperature.replace('.', '_')}")
        print("----------------------- Score Board -----------------------")
        for key in scores.keys():
            print(f"{key}: {scores[key]}")
        
        with open(f"{method}_score_result/{model_path}_temp_{temperature.replace('.', '_')}/{pi}.json", "w") as f:
            json.dump(scores, f, indent=4)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate predictions and save results to CSV.")
    parser.add_argument('--cats', nargs='*', type=list, default=['MATH', 'GSM8K', 'OMNI_MATH', "MMMLU", "KSM"], help="List of dataset categories to process, separated by spaces.")
    parser.add_argument('--model_name', type=str, default='gpt-4o', help="Name of the model to use for generating predictions.")
    parser.add_argument('--prompt_id', nargs="*", type=list, default="default", help="Prompt to use for eval.")
    parser.add_argument('--temperature', type=float, default=0.7, help="Temperature value used for model inference")
    ...
    parser.add_argumnet('--score_type', nargs="*", type=list, default=["original", "math_verify"], help="Scoring type")
